...
class GstreamerCameraBroadcaster:
    """Camera class using libcamera and GStreamer for broadcasting over WiFi."""
    
    def __init__(self, broadcast_ip="0.0.0.0", broadcast_port=5000, queue_size=2):
        ...
        # Start streaming process for network broadcast
        self.stream_process = self.start_network_stream()
        ...
        # Open GStreamer pipeline for local processing (optional)
        self.cap = self.receive_local_stream()
        
        # Check if pipeline opened successfully
        if not self.cap.isOpened():
            print("WARNING: Could not open local GStreamer pipeline for processing!")
            print("Network broadcast will continue, but local processing disabled.")
            self.local_processing = False
        else:
            print("Local GStreamer pipeline successfully opened")
    ...
    def start_network_stream(self):
        """Start libcamera-vid streaming process with network broadcast."""
        print(f"Starting network broadcast to {self.broadcast_ip}:{self.broadcast_port}")
        ...
        --inline \
        -o - | \
        gst-launch-1.0 fdsrc ! queue max-size-buffers=0 ! h264parse ! rtph264pay config-interval=1 ! udpsink host={self.broadcast_ip} port={self.broadcast_port} sync=false
        """
        
        print("Network broadcast command:")
        print(cmd)
        
        return subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    
    def receive_local_stream(self):
        """Create GStreamer pipeline for local processing (optional)."""
        # Create a separate local stream on different port for processing
        local_port = self.broadcast_port + 1
        
        # Start a secondary stream for local processing
        cmd = f"""
        libcamera-vid -t 0 \
        --nopreview \
        --width 640 --height 480 \
        ...
        --inline \
        -o - | \
        gst-launch-1.0 fdsrc ! queue ! h264parse ! rtph264pay config-interval=1 ! udpsink host=127.0.0.1 port={local_port} sync=false
        """
        
        self.local_stream_process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        time.sleep(2)  # Wait for stream to start
        
        pipeline = (
            f"udpsrc port={local_port} buffer-size=65536 timeout=0 ! "
        ...
        print("Local processing pipeline:", pipeline)
        return cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
    
    def _update(self):
        """Background thread function to continuously grab frames for local processing."""
        ...
        while self.running:
            ...
            if ret:
                ...
                print("WARNING: Failed to read frame from local GStreamer pipeline")
    ...
    def read(self):
        """Read a frame from the queue for local processing."""
        if not self.local_processing:
            return False, None
    ...
    def get_broadcast_info(self):
        ...
        return {
            ...
            'status': 'running' if self.stream_process.poll() is None else 'stopped'
        }
    ...
    def release(self):
        ...
        processes = ['stream_process', 'local_stream_process']
...
class ShapeDetector:
    ...
    def process_frame(self, frame):
        ...
        # Add FPS counter and broadcast info
        cv2.putText(frame, f"FPS: {fps:.1f}", (10, 30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        cv2.putText(frame, "BROADCASTING", (10, 60), 
...
def main():
    """Main function optimized for Raspberry Pi performance with network broadcasting."""
    ...
    broadcast_ip = "0.0.0.0"  # Broadcast to all interfaces
    broadcast_port = 5000
    
    print(f"Starting video broadcast on {broadcast_ip}:{broadcast_port}")
    print("Other devices can connect to this stream using the receiver script")
    
    # Initialize camera with GStreamer pipeline and network broadcasting
    try:
        capture = GstreamerCameraBroadcaster(broadcast_ip, broadcast_port)
        
        # Allow camera to warm up
        time.sleep(3.0)
        
        # Get broadcast info
        info = capture.get_broadcast_info()
        print(f"Broadcast Status: {info['status']}")
        print(f"Stream URL: udp://{info['ip']}:{info['port']}")
        
        # Initialize shape detector
        detector = ShapeDetector()
        
        print("Optimized Shape Detection System with Network Broadcasting Running...")
        print("Press 'q' to quit")
        
        # Main processing loop
        frame_count = 0
        while True:
            if capture.local_processing:
                # Read frame for local processing
                ...
                if ret and frame is not None:
                    ...
                    cv2.imshow('Shape Detection (Broadcasting)', result_frame)
                else:
                    print("Warning: No frame for local processing")
                    time.sleep(0.1)
            else:
                print(f"Broadcasting only... Frame {frame_count}")
                time.sleep(1.0)
                frame_count += 1
        ...
        print("Shape Detection and Broadcasting System Stopped.")
