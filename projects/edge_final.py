...
def setup_video_writers():
    """Set up video writers for raw and processed video recording."""
    # Create logs directory if it doesn't exist
    if not os.path.exists('logs'):
        os.makedirs('logs')
    
    # Create video filenames with current timestamp
    current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    raw_video_filename = f"logs/raw_video_{current_time}.mp4"
    processed_video_filename = f"logs/processed_video_{current_time}.mp4"
    
    # Define the codec and create VideoWriter objects
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    
    # Create video writers
    raw_video_writer = cv2.VideoWriter(raw_video_filename, fourcc, FPS, (WIDTH, HEIGHT))
    processed_video_writer = cv2.VideoWriter(processed_video_filename, fourcc, FPS, (WIDTH, HEIGHT))
    
    if not raw_video_writer.isOpened():
        logger.error(f"Failed to open raw video writer: {raw_video_filename}")
        raw_video_writer = None
    else:
        logger.info(f"Raw video recording started: {raw_video_filename}")
    
    if not processed_video_writer.isOpened():
        logger.error(f"Failed to open processed video writer: {processed_video_filename}")
        processed_video_writer = None
    else:
        logger.info(f"Processed video recording started: {processed_video_filename}")
    
    return raw_video_writer, processed_video_writer
...
def main():
    ...
    raw_video_writer, processed_video_writer = setup_video_writers()
    ...
    try:
        ...
        while True:
            ...
            if processed_video_writer is not None:
                processed_video_writer.write(result_frame)
        ...
        if 'raw_video_writer' in locals() and raw_video_writer is not None:
            raw_video_writer.release()
            logger.info("Raw video recording stopped and saved")
        
        if 'processed_video_writer' in locals() and processed_video_writer is not None:
            processed_video_writer.release()
            logger.info("Processed video recording stopped and saved")
